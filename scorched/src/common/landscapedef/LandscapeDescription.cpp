...
LandscapeDescription::LandscapeDescription(
	...
	descriptionNumber_(descriptionNumber)
{
}

bool LandscapeDescription::writeMessage(NetBuffer &buffer)
{
	buffer.addToBuffer(seed_);
	buffer.addToBuffer(tex_);
	buffer.addToBuffer(defn_);
	buffer.addToBuffer(name_);
	buffer.addToBuffer(descriptionNumber_);
	return true;
}

bool LandscapeDescription::readMessage(NetBufferReader &reader)
{
	...
	if (!reader.getFromBuffer(descriptionNumber_)) return false;
	return true;
}
