	...
	skipAll_(false)
{
	...
	nowId_ = addWidget(new GLWTextButton(LANG_RESOURCE("Skip Now"), 10, 10, 130, this, 
		GLWButton::ButtonFlagCenterX))->getId();
	cancelId_ = addWidget(new GLWTextButton(LANG_RESOURCE("Cancel"), 155, 10, 105, this, 
...
void SkipAllDialog::simulate(float frameTime)
{
	if (skipAll_)
	{
		...
		{
			label_->setText(LANG_RESOURCE_1("Skipping move in %i...", S3D::formatStringBuffer("%i", (5 - passedTime))));
		}
	}
